import { Cookie, CookieJar } from "another_cookiejar";
...
export function wrapFetch(options?: WrapFetchOptions): typeof fetch {
  ...
  async function wrappedFetch(input: RequestInfo | URL, init?: ExtendedRequestInit): Promise<Response> {
    ...
    const originalRequestUrl: string = (input as Request).url || input.toString();
    ...
    response.headers.forEach((value, key) => {
      if (key.toLowerCase() === "set-cookie") {
        ...
        if (value.toLowerCase().includes("expires") && expires - Date.now() < 0) return;
    ...
    const redirectUrl = response.headers.has("location")
      ? new URL(response.headers.get("location")!.toString(), originalRequestUrl).toString()
      : undefined;
    ...
    if (redirectCount >= MAX_REDIRECT) {
      await response.body?.cancel();
      throw new TypeError(`Reached maximum redirect of ${MAX_REDIRECT} for URL: ${response.url}`);
    }
...
globalThis.fetch = wrapFetch({ fetch: originalFetch, cookieJar });
